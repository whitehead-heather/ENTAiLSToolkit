...
* A single raw data file containing scans with both positive and negative polarity data should have the positive and negative mode scans isolated prior to feature list building with MZmine3. Isolate scans by polarity by running MSConvert twice using the Subset Filter for Scan Polarity
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 16.** MZmine3 main window.", fig.align = "center"}
...
Crop filtering is defined in the MZmine3 documentation as:  
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 17.** Crop filter dialog box in MZmine3.", fig.align = "center"}
...
Mass detection is defined in the MZmine3 documentation as:
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 18.** Mass detection dialog window in MZmine3.", fig.align = "center"}
...
The chromatogram building module is described in the MZmine3 documentation as: 
...
The chromatogram resolving module is described in the MZmine3 documentation 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 22.** Local minimum chromatogram resolver dialog box in MZmine3.", fig.align = "center"}
...
>  * The remaining parameters are described briefly here but are described in greater detail in the MZmine3 documentation, which can be found [here](https://mzmine.github.io/mzmine_documentation/module_docs/featdet_resolver_local_minimum/local-minimum-resolver.html)  
...
The 13C Isotope Filter module is described in the MZmine3 documentation as:
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 23.** 13C isotope filter dialog box in MZmine3.", fig.align = "center"}
...
**7. Perform Isotope Pattern Finder**  
The Isotope Pattern Finder module is described in the MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 24.** Isotope pattern finder dialog box in MZmine3.", fig.align = "center"}
...
**8. Perform Alignment**  
The Join Aligner module is described in the MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 25.** Join Aligner dialog box in MZmine3.", fig.align = "center"}
...
Gap-filling is described in the MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 26.** Gap-Filling dialog box in MZmine3.", fig.align = "center"}
...
Duplicate peak filtering is described in the MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 27.** Duplicate Peak Filtering dialog box in MZmine3.", fig.align = "center"}
...
Local Compound Database Search is described in the MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 28.** Precursor Mass List Search dialog box in MZmine3.", fig.align = "center"}
...
The Spectral Library Search is defined in MZmine3 documentation as: 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 29.** Import Spectral Library dialog box in MZmine3.", fig.align = "center"}
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 30.** Spectral Library Search dialog box in MZmine3.", fig.align = "center"}
...
>     * See the MZmine3 documentation for more information
...
There are multiple options for exporting data out of MZmine that include export to CSV (discussed here), and export for integration into other software or tools as described in the [MZmine3 documentation](https://mzmine.github.io/mzmine_documentation/tool_integration.html). For CSV exports there are several options to export feature lists out of MZmine for use in Excel, R, or other software. Here, two different export options are described. 
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 31.** Export CSV dialog box in MZmine3.", fig.align = "center"}
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 32.** Export CSV dialog box in MZmine3.", fig.align = "center"}
...
To return to processed data in MZmine3 without re-processing data save the work as a project.
...
MZmine3Output %>%
kable(caption ="**Table 9.** Description of input parameters and files for the EPA NTA WebApp.",format="html", align="l",html_font="Lato") %>%
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive"))
```

The primary input to the MS1 workflow is a detection matrix containing a list of MS1 features with required columns (**Table 10**), such as mass or m/z, retention time, and instrument responses (e.g., peak areas) across a set of samples. Feature level information columns are typically given first in the input, though an exact order is not required. Regarding instrument responses across samples, submitted values should be numeric, with an appropriate level of precision based on the utilized method and upstream data processing software. Both empty cells and cells containing a numeric value of “0” are treated as non-detects. Because of the logic and filtering steps performed, instrument responses should not be imputed, normalized, or otherwise transformed prior to WebApp processing. 

Columns containing instrument responses of features in samples must be named such that sample and blank replicates can be identified by the WebApp. The identification of replicate samples is done with single-character differences, for example “MySample_a”, “MySample_b”, and “MySample_c” would be identified as three replicates of a single sample with the name “MySample”. Single-character differences may be letters as shown here or as numbers. Additional columns (e.g., initial annotations from upstream data processing) beyond those described above may also be included in the input detection matrix. These columns are treated as pass-through columns that are unaltered during WebApp processing. A minimal example detection matrix is attached below. This example matrix contains a limited number of features and samples but includes all required inputs. One additional column, “CompoundID”, is included to track the unique feature IDs assigned during upstream processing. This variable is treated by the WebApp as a pass-through column.
...
After submission of a WebApp job, the user is directed to a new webpage where a unique job identifier and status update are displayed. Successful runs will return a “get results” button that links to a .zip file containing the complete results.  
...
* Tracer plot(s) identified by "tracer_plot_MODE_#.png"
  * Note that MODE will be given as either "neg" or "pos" depending on which data were loaded into WebApp and if there were tracers for that mode
  * The number identifies when there are multiple plots generated. The WebApp will put up to 16 tracers in a single .png. When more tracers are present, more plots are generated
  
If the WebApp is run multiple times, it is important to note that unique .zip files will be generated for each run. An example .zip file containing the output from the example input is attached, and the individual output files are described in detail below.  

```{r echo=FALSE}
xfun::pkg_load2(c("htmltools", "mime"))
xfun::embed_file("WebAppOutput.zip")
```
  
**_CV Scatter Plot_**  
The CV scatter plot (example shown in **Figure 34**) contains two subplots representing the mean abundance versus the measured CV of each sample occurrence. Information on the left subplot shows occurrences in samples that were identified as the "Blanks" in the WebApp input files, while the right subplot shows occurrences in all other samples. In each plot, the x-axis is dynamically adjusted to reflect the range of mean abundances across all samples, while the y-axis is fixed to a CV range of 0 – 2.5. In each subplot, a red-dashed line identifies the CV threshold set by the user when inputting parameters; bolded text above the line shows the exact CV value used. Red dots identify sample occurrences linked to tracers (from the tracer input file); all other occurrences are grey.
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 34.** Example CV scatter plot produced by WebApp."}
...
* The sample identified as the "blank" for WebApp processing should show no detections (completely gray row).
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 35.** Example occurrence heatmap produced by WebApp."}
...
MZmine3Output %>%
kable(caption ="**Table 13.** Description of sheets and their contents from the WebApp Excel output.",format="html", align="l",html_font="Lato") %>%
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive"))
```
...
In this protocol, only FluoroMatch Modular is used, with the WebApp filtered feature lists as input. FluoroMatch Modular contains functional elements of FluoroMatch that run as scripts rather than through the FluoroMatch graphical interface.   
...
Peak lists can be generated by various software including both vendor specific or proprietary tools and by third-party open source software like MZmine3. FluoroMatch Modular requires a peak list to be input as a CSV file that has the general structure of samples (columns) against features (rows) that includes:   
...
This protocol focuses on the use of MZmine3 outputs as the input peak list for FluoroMatch. When EPA’s NTA WebApp has been used to perform QA/QC filtering on the feature list, the output filtered feature list from WebApp should be used as the input peak list. An example output feature list from MZmine3 using the “CSV (Legacy from MZmine3)” export is shown in **Table 14** for reference. This feature list has been reduced to just 10 features in 3 samples for brevity. All required elements for FluoroMatch processing of this input peak list, as described above, are present in the MZmine3 output. This file can be used without modification of its contents.  
...
MZmine3Output %>%
kable(caption ="**Table 14.** Example MZmine3 output for preparation.",format="html", align="l",html_font="Lato") %>%
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive"))
```
...
Similar to requirements for MZmine3, FluoroMatch requires raw data files to be converted into a vendor-neutral file format, here .ms2, for processing with FluoroMatch Modular. 
...
```{r, echo=FALSE, out.width="75%", fig.cap="**Figure 42.** Comparison of experimental spectra (top plot) with reference spectra (bottom plot) from spectral libary searching in MZmine 3."}
...
FluoroMatchOutput2 %>%
kable(caption ="**Table 18.** Parameters used to perform ion identity and spectral similarity networking in MZmine 3.'",format="html", align="l",html_font="Lato") %>%
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive"))
```

Outputs from these networks can be visualized in MZmine 3 as shown in **Figure 45** below but may also be exported as annotations in the export from MZmine.
...
* The use of other data processing tools may introduce the need for additional review to ensure their accuracy in use.
  * For example, the processes performed, and annotations generated, by EPA's NTA WebApp have additional considerations not described here.
...
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive")) %>%
  ...
  pack_rows("MZmine 3 (3.9.0) [DOWNLOAD](https://github.com/mzmine/mzmine/releases)", 3, 6,label_row_css = "background-color: #0668b3; color: #fff;") %>%
  pack_rows("EPA NTA WebApp *(Beta tool in development)*", 7, 8,label_row_css = "background-color: #0668b3; color: #fff;") %>%
...
## Appendix E: Summarized MZmine 3 modules and parameters described
```{r, echo=FALSE}
library(knitr)
library(kableExtra)
...
FluoroMatchOutput2 %>%
kable(caption ="**APPENDIX E**. Summarized MZmine 3 modules and parameters described. . ",format="html", align="c",html_font="Lato") %>%
kable_styling(full_width=F, html_font= "Lato", bootstrap_options=c("striped", "hover","responsive")) %>%
...
## Contact
For any questions or comments on the toolkit content or the ROAR project please reach out to **Heather Whitehead** at **whitehead.heather@epa.gov.** 